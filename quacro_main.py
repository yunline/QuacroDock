...
quacro_logging.set_except_hook(logger)
...
if result==quacro_c_utils.ACQUIRE_SILOCK_SUCCESS:
    ...
    raise RuntimeError(
        f"Failed to acquire single instance lock, return code: {result}"
    )
...
try:
    ...
    logger.error(f"Error when load config: {err}")
    # todo:i18n
...
try:
    ...
    logger.error(f"Error when load window filter config: {err}")
    # todo:i18n
    quacro_win32.fatal_msgbox(
